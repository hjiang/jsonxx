...
NOTE: For older compilers that do not support C++11, please use the “legacy” branch.
...
languages these days, none of the existing C++ JSON parsers suit my
needs very well, so I wrote one that I used in another project. My goals
for JSON++ are:
...
* Conversion from JSON to XML: JSONx flavour. See http://goo.gl/I3cxs for details.
* Conversion from JSON to XML: JXML flavour. See https://github.com/r-lyeh/JXML for details.
* Conversion from JSON to XML: JXMLex flavour. See https://github.com/r-lyeh/JXMLex for details.
...
cout << o.xml(jsonx) << endl;                 // XML conversion (JSONx)
cout << o.xml(jxml) << endl;                  // XML conversion (JXML)
cout << o.xml(jxmlex) << endl;                // XML conversion (JXMLex)
</code>
</pre>
