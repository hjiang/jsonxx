...
h1. Notes about jsonxx::Settings values

h2. Strict/permissive parsing
JSONxx can parse JSON documents both in strict or permissive mode.

When strict parsing is enabled, JSONxx parser will accept:
* Fully conformant JSON documents *only*

When strict parsing is disabled, JSONxx parser will accept:
* Fully conformant JSON documents, plus following *three additions*
...
Set `jsonxx::Settings::Strict` value to `Enabled` or `Disabled` to enable or disable strict parsing. Default value is `Disabled`.

h2. Assertions
JSONxx uses internally `JSONXX_ASSERT(...)` macro that works both in debug and release mode.

Set `jsonxx::Settings::Assertions` value to `Disabled` to disable assertions. Default value is `Enabled`.
